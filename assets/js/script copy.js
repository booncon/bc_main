...
function initUI() {
  ...
  $('.tools').click(function(e) {
    e.preventDefault();
    scrollSmooth($('#projects').offset().top);
  });
  $('.header_logo').click(function () {
    $.bbq.removeState();
    location.reload();
  });
  $('.message_status').delay(8000).hide("blind", { direction: "vertical" }, anim_dur);  //fade-out successful e-mail-message  
  $('.retina').retina();
  $('.tech_toggle').tipsy({gravity: 's'}).click(function () {
    return false;
  });
}
...
function loadProject(url) {
  ...
  $('.explode').load(url + ' #slide_wrapper', function () {
    var img_y, cur_proj, count_proj, next_proj, prev_proj, loop_proj;  
...
function projectPreloader(url) {
  if ($('.explode.open').length === 0) { //no project open
    ...
    $('body').animate({scrollTop: $('#project_wrapper').offset().top - 60}, anim_dur, function () {
      $('#slide_wrapper').hide("drop", { direction: "up" }, anim_dur, function () {      
        $(this).empty();
        loadProject(url);
      }); 
    });         
...
function initShowcase() {
  $('.showcase_block').click(function () {
    var ajax_url, slider_is_close;
    slider_is_close = false;
