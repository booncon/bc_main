...
function resizeSlider() {
  ...
  if ((navigator.userAgent.match(/iPhone/i) !== null) || (navigator.userAgent.match(/iPod/i) !== null)) {
    slider_height -= 80;  //fix window height on iphone / + top bar
  } else {
    if (navigator.userAgent.match(/iPad/i) === null) {
      slider_height -= $('#sticky_menu').height();
      slider_height *= 0.72;
    } else {
      slider_height *= 0.64;
    }    
  }  
  $('.wallpaper').height(slider_height);
}
...
function scrollSmooth(position) {
  if ((navigator.userAgent.match(/iPhone/i) !== null) || (navigator.userAgent.match(/iPod/i) !== null) || (navigator.userAgent.match(/iPad/i) !== null)) {
    position += 50; //ios devices do not have sticky top bar
  }
  $('body').animate({scrollTop: position}, anim_dur);
}
...
function initKeyHandler() {
  $(document).unbind('keydown').bind('keydown', function(e) {
    ...
    switch (code) {
      ...
      break;
      case 37: //arrow left
        $('.wmuSliderPrev:visible').trigger('click');
      break;
      case 39: //arrow right
        $('.wmuSliderNext:visible').trigger('click');
      break;
      case 32: //space
        $('.wmuSliderNext:visible').trigger('click');
      break;
...
function loadProject(url, first) {
  ...
  $('.explode').load(url + ' #slide_wrapper', function () {
    ...
    $('.wmuSlider').wmuSlider({
      ...
      touch: false,
      teasingFactor: 2.2,
      teasingOverlapLimit: 56,
      previousText: 'navigateleft',
      nextText: 'navigateright'
    });
    //close button
...
function initD() {
  ...
  // Adjust font size to the radius.
  var font_size_scale = d3.scale.linear().domain([50,100]).range([12, 25]);
  var text_length_scale = d3.scale.linear().domain([5,20]).range([1.4, 0.65]);
  ...
  d3_nodes.append("text")
      ...
      .style("font-size", function(d) { return font_size_scale(d.r * text_length_scale(d.className.length)); } )
      .text(function(d) { return d.className; });
...
function initJQ() {
  ...
  // Append hash anchor to translation links
  $('a', '.lang_menu').click(function() {
